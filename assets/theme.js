...
function invertHeader(trigger) {
  ...
  elements.forEach(element => {
    ...
    if (!inversed && bounding.top <= header.clientHeight / 2 && bounding.bottom >= header.clientHeight / 2) {
      header.classList.add('inversed');
      parent.dataset.inversed = true;
      inversed = true;
    } else if (trigger) {
      const currentBounding = document.getElementById(trigger) && document.getElementById(trigger).getBoundingClientRect();
      if (currentBounding && currentBounding.top <= header.clientHeight / 2 && currentBounding.bottom >= header.clientHeight / 2) {
        header.classList.add('inversed');
      } else {
...
window.addEventListener('scroll', function(evt){
  ...
  document.querySelector('[data-inversed="true"]') ? inversionTriggerId = document.querySelector('[data-inversed="true"]').id : null;
    ...
    tabs.forEach(tabSection => {
      ...
      buttons.forEach(button => {
        button.addEventListener('click', function() {
          const target = tabSection.querySelector(`.tabs-contents [data-index="${button.dataset.index}"]`);
