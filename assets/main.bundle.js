...
const LOAD_ATTRIBUTES_DOM = "data-load-dom",
    ...
    SELECTORS$1_LAZY_MEDIA = '[loading="lazy"], [data-src], [data-poster], [data-srcset]',
    SELECTORS$1_PICTURE = "PICTURE";

function setMediaAttr(el) {
    el.dataset.src && !el.src && (el.src = el.dataset.src), el.dataset.srcset && !el.srcset && (el.srcset = el.dataset.srcset), el.dataset.poster && !el.poster && (el.poster = el.dataset.poster)
}

function setMediaSources(media) {
    var _a;
    if (media) try {
        (null == (_a = media.parentNode) ? void 0 : _a.tagName) === SELECTORS$1_PICTURE ? Array.from(media.parentNode.children).forEach((el => {
            setMediaAttr(el)
        })) : setMediaAttr(media)
    } catch (error) {
...
function initLazyMedia() {
    ...
    if (document.querySelectorAll(SELECTORS$1_LAZY_MEDIA).forEach((media => {
            if (media.hasAttribute(LOAD_ATTRIBUTES_DOM)) mediaOnDOMLoad.push(media);
            else if (media.hasAttribute(LOAD_ATTRIBUTES_LOAD)) mediaOnLoad.push(media);
            else {
                if (media.hasAttribute(LOAD_ATTRIBUTES_MANUAL)) return;
                "loading" in HTMLImageElement.prototype == !1 && "IntersectionObserver" in window ? mediaForIntersectionObserver.push(media) : setMediaSources(media)
            }
        ...
        }(mediaOnDOMLoad), function loadMediaOnLoaded(mediaElArray) {
            const execute = () => afterCallstack((() => setMediaSourceArray(mediaElArray)));
            if ("complete" == document.readyState) return execute();
            window.addEventListener("load", execute)
        }(mediaOnLoad), mediaForIntersectionObserver.length > 0) {
...
afterCallstack(initLazyMedia), document.addEventListener("DOMContentLoaded", initLazyMedia), document.addEventListener("shopify:section:load", (() => afterCallstack(initLazyMedia)));
const initLazyImages = initLazyMedia,
    _ReloadOnEvent = class _ReloadOnEvent extends HTMLElement {
        constructor() {
            super()
        }
...
class ResponsiveVideo extends HTMLElement {
    constructor() {
        super(), this.onClick = this.onClick.bind(this), this.onResize = this.onResize.bind(this), this.onLoaded = this.onLoaded.bind(this)
    }
    connectedCallback() {
        this.lazyType = this.getAttribute("data-lazy-type"), this.initElements(), this.sources = this.getSources(), this.video.style.opacity = "0", this.videoWrapper.addEventListener("click", this.onClick), this.video.addEventListener("loadeddata", this.onLoaded), window.addEventListener("resize", this.onResize), this.initVideo()
    }
    initVideo() {
        this.updateSource();
        const execute = () => {
            this.lazyType ? (this.video.setAttribute("data-src", this.currentSource), initLazyMedia()) : this.load()
        };
    ...
    cleanSourceURL(src) {
        return src.startsWith("//") && (src = `${window.location.protocol}${src}`), new URL(src, window.location.href).href
    }
    updateSource() {
    ...
    onClick(e) {
        e.target.closest("[data-sound-toggle]") ? this.toggleSound() : this.togglePlay()
    }
    togglePlay() {
        this.video.paused ? this.video.play().catch((() => {})) : this.video.pause(), this.updateControlsUI()
    }
    toggleSound() {
        this.video.muted = !this.video.muted, this.updateControlsUI()
    }
    onLoaded() {
        this.poster && (this.poster.style.opacity = "0"), this.video.style.opacity = "1", this.video.hasAttribute("autoplay") && (this.video.muted = !0, this.video.play().catch((() => {}))), this.updateControlsUI()
    }
    updateControlsUI() {
        this.playIcon && this.playIcon.classList.toggle("hidden", !this.video.paused), this.soundOnIcon && this.soundOffIcon && (this.soundOnIcon.classList.toggle("hidden", this.video.muted), this.soundOffIcon.classList.toggle("hidden", !this.video.muted))
    }
    disconnectedCallback() {
        var _a;
        this.removeEventListener("click", this.onClick), null == (_a = this.soundBtn) || _a.removeEventListener("click", this.onSound), this.video.removeEventListener("loadeddata", this.onLoaded), window.removeEventListener("resize", this.onResize)
    }
}
...
class SideDrawer extends HTMLElement {
    ...
    connectedCallback() {
        var _a;
        this.init(), this.sectionId = null == (_a = this.closest(".shopify-section")) ? void 0 : _a.id.replace("shopify-section-", ""), Shopify.designMode && (document.addEventListener("shopify:section:select", this.handleSectionSelect.bind(this)), document.addEventListener("shopify:section:deselect", this.handleSectionDeselect.bind(this)))
    }
    disconnectedCallback() {
        Shopify.designMode && (document.removeEventListener("shopify:section:select", this.handleSectionSelect.bind(this)), document.removeEventListener("shopify:section:deselect", this.handleSectionDeselect.bind(this)))
    }
    init() {
        this.toggleSelector = `[${this.dataset.toggleAttribute}]`, document.querySelectorAll(this.toggleSelector).forEach((el => {
            this.contains(el) || el.addEventListener("click", (() => {
                this.toggle()
            }))
        })), this.addEventListener("click", this.handleDrawerClick.bind(this)), this.addEventListener("keyup", (evt => {
            "Escape" === evt.key && this.close()
        }))
    }
    handleDrawerClick(evt) {
        evt.target.closest(this.toggleSelector) && (evt.preventDefault(), this.toggle())
    }
    handleSectionSelect(event) {
        const eventSectionId = event.detail.sectionId;
        this.sectionId && eventSectionId === this.sectionId && this.open()
    }
    handleSectionDeselect(event) {
        const eventSectionId = event.detail.sectionId;
        this.sectionId && eventSectionId === this.sectionId && this.close()
    }
    get isOpen() {
    ...
    open() {
        this.toggleAttribute("open", !0),
            function lockBodyScroll() {
                window.unlockedScrollY = window.scrollY, console.log("locking body scroll at", window.unlockedScrollY), document.body.style.overflow = "hidden", document.body.style.position = "fixed", document.body.style.top = `-${window.unlockedScrollY}px`, document.body.style.width = "100%", document.body.style.paddingRight = "0px"
            }()
    }
    close() {
        this.toggleAttribute("open", !1),
            function unlockBodyScroll() {
                ...
                document.body.style.overflow = "", document.body.style.position = "", document.body.style.top = "", document.body.style.width = "", document.body.style.paddingRight = "", window.scrollTo(0, scrollY), delete window.unlockedScrollY
            }()
    }
}
customElements.define("side-drawer", SideDrawer);
const _CartDrawer = class _CartDrawer extends SideDrawer {
    constructor() {
        super()
    }
    init() {
        super.init(), document.addEventListener("on:cart:add", this.handleCartAddEvent.bind(this)), document.addEventListener("cart:add:loading:start", this.handleCartAddLoadingStart.bind(this)), this.addEventListener("change", function debounce(fn, wait = 300) {
            let t;
            return (...args) => {
                clearTimeout(t), t = setTimeout((() => fn.apply(this, args)), wait)
            }
        }(this.handleChange.bind(this))), this.addEventListener("click", this.handleClick.bind(this))
    }
    handleChange(evt) {
    ...
    handleClick(evt) {
        ...
        button && (evt.preventDefault(), this.updateQuantity(button.dataset.index, 0))
    }
    async updateQuantity(line, quantity) {
        try {
            this.querySelector(`#line-item-${line}`).classList.add("is-loading");
    ...
    handleCartAddEvent(e) {
        var _a, _b;
        (null == (_a = e.detail) ? void 0 : _a.sections) && (null == (_b = e.detail) ? void 0 : _b.sections["cart-drawer"]) ? this.renderContents(e.detail.sections): this.refresh(), this.classList.remove("cart-loading")
    }
    ...
    renderContents(sections, openCart = !0) {
        sections["cart-drawer"] && (this.innerHTML = _CartDrawer.getSafeElementHTML(sections["cart-drawer"]), openCart && !this.isOpen && setTimeout((() => this.open()))), sections["cart-icon-bubble"] && _CartDrawer.updateCartIcon(sections), initLazyMedia()
    }
    static getSafeElementHTML(html) {
        const tmpl = document.createElement("template");
        return tmpl.innerHTML = html, tmpl.content.querySelector("cart-drawer").innerHTML
    }
    static updateCartIcon(sections) {
        const cartIconBubble = document.getElementById("cart-icon-bubble");
        cartIconBubble && (cartIconBubble.innerHTML = sections["cart-icon-bubble"])
    }
};
__publicField(_CartDrawer, "sections", ["cart-drawer", "cart-icon-bubble"]);
let CartDrawer = _CartDrawer;
...
class QuantityInput extends HTMLElement {
    ...
    connectedCallback() {
        this.input = this.querySelector("input"), this.currentQty = this.input.value, this.changeEvent = new Event("change", {
            bubbles: !0
        }), this.addEventListener("click", this.handleClick.bind(this)), this.input.addEventListener("keydown", this.handleKeydown.bind(this))
    }
    handleClick(evt) {
...
customElements.define("quantity-input", QuantityInput), document.addEventListener("DOMContentLoaded", (() => {
    document.addEventListener("click", (evt => {
        const link = "A" === evt.target.tagName ? evt.target : evt.target.closest("a");
        link && "A" === link.tagName && window.location.hostname !== new URL(link.href).hostname && (link.target = "_blank")
    })), document.addEventListener("click", (evt => {
        "A" === evt.target.tagName && window.location.hostname === new URL(evt.target.href).hostname && evt.target.href.includes("#") && (document.getElementsByTagName("html")[0].style.scrollBehavior = "smooth", setTimeout((() => {
...
(function () {
  function shopifyPath(path) {
    // path: 'cart/add.js' | '/cart/add.js' | 'cart'
    var root = (window.Shopify && window.Shopify.routes && window.Shopify.routes.root) ? window.Shopify.routes.root : '/';
    root = root.replace(/\/+$/,'');                 // '/en-us/' -> '/en-us', '/' -> ''
    path = String(path || '').replace(/^\/+/, '');  // '/cart/add.js' -> 'cart/add.js'
    // '/cart/add.js'; '/en-us' → '/en-us/cart/add.js'
    return '/' + (root ? (root.replace(/^\//,'') + '/') : '') + path;
  }
  ...
  function getVariantIdFromCustomSelect(form) {
    ...
    const opt = Array.from(cs.querySelectorAll('.custom-option')).find(o =>
      o.dataset?.title?.trim() === selectedTitle || o.textContent?.trim() === selectedTitle
    );
  ...
  document.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const url = shopifyPath('cart/add.js');
      ...
      location.href = shopifyPath('cart');
    }
  }, true);
})();
