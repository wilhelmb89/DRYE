...
function updateQty(id, qty, row) {
    ...
    fetch(window.Shopify.routes.root + 'cart/update.js', {
        ...
        .then((data) => {
            ...
            if (liveBubble) {
                liveBubble.querySelector('#liveBubble .cart').innerHTML =
                    resHeader.querySelector('#liveBubble .cart').innerHTML;
            }

            // Update drawer and main cart
            if (liveDrawer.querySelector('.row')) {
                liveDrawer.querySelector('.below-qty').innerHTML =
                    resDrawer.querySelector('.below-qty').innerHTML;
                liveDrawer.querySelector('.cart-body').innerHTML =
                    resDrawer.querySelector('.cart-body').innerHTML;
                liveDrawer.querySelector('.cart-footer').innerHTML =
                    resDrawer.querySelector('.cart-footer').innerHTML;
            }

            if (mainCart) {
                mainCart.querySelector('.cart-body').innerHTML =
                    resCart.querySelector('.cart-body').innerHTML;
                mainCart.querySelector('.cart-footer').innerHTML =
                    resCart.querySelector('.cart-footer').innerHTML;
            }
        });
}
